...
class ImgTransformation():
    ...
    def color_change(self, file_name):
        src = cv2.imread(os.path.join(self.data_path, f'val2017/{file_name}'), cv2.IMREAD_UNCHANGED)
        red_img, green_img, blue_img = np.zeros(src.shape), np.zeros(src.shape), np.zeros(src.shape)
