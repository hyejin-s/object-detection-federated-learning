...
def main(args):
    ...
    for file in random_list:
        ...
        img_path = os.path.join(args.save_dir, f'images/random_{args.ratio}/')
        shutil.copy(
            args.data_dir+img_partition+file_name+'.jpg', os.path.join(img_path, file_name)+'.jpg'
        )
        # label copy
        label_path = os.path.join(args.save_dir, f'labels/random_{args.ratio}/')
        shutil.copy(
             args.data_dir+partition+file_name+'.txt',  os.path.join(label_path, file_name)+'.txt'
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ratio",
        help="copy ratio", type=float, default=0.5,
    )
