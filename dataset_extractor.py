...
class CocoCustomizeExcept():
    ...
    def collect_data(self, collect_class_list: list):
        ...
        for file in tqdm(self.file_list):
            after_remove_list = self.class_removal(file, collect_class_list) ### file 안에서 class 있는지 확인
            ...
            if len(after_remove_list) != 0: ### 남아 있는 게 있으면
                ...
                for obj in obj_list_uniq:
                    labels_list = [i for i in after_remove_list if int(i.split()[0]) == obj]
                    with open(os.path.join(args.root, f"data/class{obj}/labels/{file}"), "w+") as f:
                        for j in labels_list:
                            f.write(j)
                            f.write("\n")
                    img_path = os.path.join(args.root, f"data/class{obj}/images/")
                    img_name = file.split('.')[0] + ".jpg"
                    shutil.copy(args.data_path+f"images/train2017/{img_name}", img_path)
